...
class Client:
	...
	async def handle_packet(self, connection: Connection, packet: Packet) -> bool:
		...
		if CCC == (1, 1): # Old packets
			...
			self.room.map.code = packet.read32()
			packet.read16()
			self.room.round_code = packet.read8()
			packet.read16()
			...
			self.room.map.author = packet.readUTF()
			self.room.map.perm = packet.read8()

			is_reversed = packet.readBool()
			self.room.map.is_reversed = is_reversed
			
			if xml:
				self.room.map.xml = xml
				self.room.map.length = get_map_length(xml)
				self.room.map.hole_pos = get_data(xml, 'T', reversed=is_reversed)
				self.room.map.cheese_pos = get_data(xml, 'F', reversed=is_reversed)
			else:
				self.room.map = Map()
				
			self.dispatch('map_change', self.room.map)
		...
		if CCC == (5, 21): # Joined room
			...
			author = self.room.get_player(name=author, default=Player(author))
